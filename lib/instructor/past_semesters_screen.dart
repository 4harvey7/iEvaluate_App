...
class _PastSemestersScreenState extends State<PastSemestersScreen> {
      ...
      for (var item in summaries) {
        ...
        termsMap[tid] = {
          ...
          'overallScore':
              ...
              0.0,
          'managementScore':
              (item['management_mean'] as num?)?.toDouble() ?? 0.0,
          'performanceScore':
              (item['performance_mean'] as num?)?.toDouble() ?? 0.0,
        ...
        setState(() {
          ...
          if (_selectedTermId == null) {
            ...
            if (processedData.any((t) => t['termId'] == currentTermId)) {
              _selectedTermId =
                  currentTermId; // current term is in history, select it
            } else if (processedData.isNotEmpty) {
              _selectedTermId =
                  processedData.last['termId']; // just pick the latest one
            }
          }
          _isLoading = false;
        });
  ...
  Future<void> _loadSelectedTermData(String termId) async {
    try {
      ...
      final subjectsList = await _supabase
          ...
          .select(
            'subject_id, subjects(id, subject_code, subject_name, department_id)',
          )
          .eq('instructor_id', widget.userId)
          .eq('term_id', termId);
      ...
      for (var row in subjectsList) {
        ...
        codeToIds
            .putIfAbsent(code, () => [])
            .add(id); // group sections by subject code
      }
      ...
      for (var entry in codeToIds.entries) {
        ...
        final meta =
            subjectById[ids.first]!; // use first section's metadata for display
        ...
        if (mMean == 0.0 && pMean == 0.0) {
          debugPrint(
            '[PastSemesters] No precomputed data for $code in $termId, using raw...',
          );
        ...
        for (var id in ids) {
          ...
          if (mg != null) {
            debugPrint(
              '[PastSemesters]     mgmt: overall=${mg["overall_management_mean"]} | responses=${mg["total_responses"]} | created=${mg["created_at"]}',
            );
          } else {
          ...
          if (pf != null) {
            debugPrint(
              '[PastSemesters]     perf: overall=${pf["overall_performance_mean"]} | responses=${pf["total_responses"]} | created=${pf["created_at"]}',
            );
          } else {
      ...
      processed.sort(
        (a, b) => (a['subject_code'] ?? '').compareTo(b['subject_code'] ?? ''),
      );
  ...
  Map<String, dynamic>? get _selectedTermData {
    ...
    final matches = _historicalData.where(
      (t) => t['termId'] == _selectedTermId,
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ...
          : RefreshIndicator(
              onRefresh:
                  _fetchHistory, // pull-to-refresh also re-fetches history
              color: AppColors.primary,
              child: SingleChildScrollView(
                physics:
                    const AlwaysScrollableScrollPhysics(), // needed for pull-to-refresh even on short content
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text(
                      'Track your evaluation scores across previous academic terms.',
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (_) => DetailedReportScreen(
                                    ...
                                    instructorName: name,
                                    department:
                                        'Faculty', // hardcoded for now — not great but acceptable
                                    termId: _selectedTermId,
                                    // Parse semester and academic year from combined string
                                    term:
                                        termData['semester']?.toString().split(
                                          ' ',
                                        )[0] ??
                                        '',
                                    academicYear:
                                        ...
                                        : '',
                                    managementScore:
                                        (termData['managementScore'] as num?)
                                            ?.toDouble() ??
                                        0.0,
                                    performanceScore:
                                        (termData['performanceScore'] as num?)
                                            ?.toDouble() ??
                                        0.0,
                                    overallScore:
                                        (termData['overallScore'] as num?)
                                            ?.toDouble() ??
                                        0.0,
                                    totalEvaluations:
                                        (termData['evaluations'] as int?) ?? 0,
                                  ),
                                ),
                              );
                            },
                          ...
                          child: DropdownButton<String>(
                            ...
                            onChanged: (val) {
                              if (val != null) {
                                ...
                                _loadSelectedTermData(
                                  val,
                                ); // and load its subjects
                              }
                            },
                          ),
                        ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        if (!_isTermLoading)
                          Text(
                            '${_selectedTermSubjects.length} Subjects',
                    ...
                    if (_isTermLoading)
                      ...
                      const Center(
                        child: Padding(
                          padding: EdgeInsets.all(32),
                          child: Text('No subjects found for this term.'),
                        ),
                      )
                    else
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [AppColors.textPrimary, const Color(0xFF1E293B)],
        ),
                ...
                : SingleChildScrollView(
                    ...
                    child: Row(
                      ...
                      children: _historicalData.map((data) {
                        double score =
                            (data['overallScore'] as num?)?.toDouble() ?? 0.0;
                        // Bar height proportional to score out of 5 — max 120px
                        double h = (score / 5.0) * 120;
                        bool isSel =
                            data['termId'] ==
                            _selectedTermId; // highlight selected term
                        return Padding(
                              ...
                              GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  height: h.clamp(
                                    5,
                                    120,
                                  ), // minimum height 5px so it's always visible
                                  decoration: BoxDecoration(
                                    color: isSel
                                        ? AppColors.primary
                                        : Colors.white24, // highlight selected
                              ...
                              SizedBox(
                                ...
                                child: Builder(
                                  builder: (_) {
                                    ...
                                    final ordinal = parts.isNotEmpty
                                        ? parts[0]
                                        : full; // "1st"
                                    ...
                                    final label = yearShort.isNotEmpty
                                        ? '$ordinal\n$yearShort'
                                        : ordinal;
                                    return Text(
                                      label,
