...
class _ImportErrorDetailScreenState extends State<ImportErrorDetailScreen> {
  ...
  void _initScoreControllers() {
    final raw = _rawData;
    for (int i = 1; i <= 10; i++) {
      // n8n saves uppercase (M1, P1) but some paths use lowercase (m1, p1) — check both
      final mVal = raw['m$i'] ?? raw['M$i'];
      final pVal = raw['p$i'] ?? raw['P$i'];
      _scoreCtrlMap['m$i'] =
          TextEditingController(text: mVal?.toString() ?? '');
      _scoreCtrlMap['p$i'] =
          TextEditingController(text: pVal?.toString() ?? '');
    }
  }
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  // ── Error type banner ─────────────────────────────────
                  // colored banner explaining what is wrong — hard to miss
                  _buildErrorBanner(),

                  // ── Scanned image (scan errors only) ──────────────────
                  // only show if it came from a scan — sheet has no image
