      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Subject code pill — easy to identify at a glance
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
              decoration: BoxDecoration(color: AppColors.primaryTint, borderRadius: BorderRadius.circular(100)),
              child: Text(subject.code, style: const TextStyle(color: AppColors.primaryText, fontWeight: FontWeight.w700, fontSize: 12), overflow: TextOverflow.ellipsis),
            ),
            const SizedBox(height: 10),
            // Full subject name — big and bold
            Text(subject.name, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, letterSpacing: -0.5, color: AppColors.textPrimary), overflow: TextOverflow.ellipsis),
            const SizedBox(height: 24),
            const Text('Instructor Performance Breakdown', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16, color: AppColors.textPrimary)),
            const SizedBox(height: 16),
            // List of instructors teaching this subject — scrollable in case many
            Expanded(
              child: subject.instructorBreakdown.isEmpty 
                ? const Center(child: Text("No detailed instructor data available.", style: TextStyle(color: AppColors.textSecondary))) // Fallback if data missing
                : ListView.separated(
                    itemCount: subject.instructorBreakdown.length,
                    separatorBuilder: (context, index) => const Divider(color: AppColors.borderHairline), // Hairline between instructors
                      ...
                      return Padding(
                        ...
                        child: Column(
                          children: [
                            Row(
                              children: [
                                // Avatar — soft tint circle with the instructor's initial
                                CircleAvatar(
                                  radius: 20,
                                  backgroundColor: AppColors.primaryTint,
                                  child: Text(
                                    item.instructorName.isNotEmpty ? item.instructorName[0].toUpperCase() : '?',
                                    style: const TextStyle(color: AppColors.primaryText, fontWeight: FontWeight.w800, fontSize: 16),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(item.instructorName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: AppColors.textPrimary), overflow: TextOverflow.ellipsis),
                                ...
                                Column(
                                  ...
                                  children: [
                                    // Their average score — red if below 3.0, accessible orange if okay
                                    Text('${item.avgScore}', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 18, color: item.avgScore < 3.0 ? AppColors.error : AppColors.primaryText)),
                            ...
                            Stack(
                              children: [
                                // Background track — full width soft vanilla
                                Container(height: 8, width: double.infinity, decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(100))),
                                // Foreground fill — proportional to score, red if bad
                                FractionallySizedBox(
                                  widthFactor: (item.avgScore / 5.0).clamp(0.0, 1.0), // Never exceed 100%
                                  child: Container(height: 8, decoration: BoxDecoration(color: item.avgScore < 3.0 ? AppColors.error : AppColors.primary, borderRadius: BorderRadius.circular(100))),
                                ),
                              ],
                            ),
                          ],
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Subject Analytics', style: TextStyle(color: AppColors.textInverted, fontWeight: FontWeight.w800, letterSpacing: -0.5)),
      ),
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: _isLoading 
            ? const Center(child: CircularProgressIndicator(color: AppColors.primary)) // Show spinner while loading
            : SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Curriculum Health', style: TextStyle(color: AppColors.textPrimary, fontSize: 26, fontWeight: FontWeight.w800, letterSpacing: -0.5)),
                    const SizedBox(height: 6),
                    const Text('Identifying subjects that may require curriculum review or additional teaching resources.', style: TextStyle(color: AppColors.textSecondary, fontSize: 14, height: 1.4)),
                    const SizedBox(height: 24),
                    ...
                    if (_subjectAnalytics.isEmpty)
                      Center(
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              // Soft tinted icon circle — friendly empty state
                              Container(
                                width: 72,
                                height: 72,
                                decoration: const BoxDecoration(color: AppColors.primaryTint, shape: BoxShape.circle),
                                child: const Icon(Icons.menu_book_outlined, color: AppColors.primaryText, size: 32),
                              ),
                              const SizedBox(height: 16),
                              const Text("No subject data available for this term.", style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
                            ],
                          ),
                        ),
                      )
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          // Subject code chip — pill shape, tint fill + accessible orange label
                                          Container(
                                            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                                            decoration: BoxDecoration(color: AppColors.primaryTint, borderRadius: BorderRadius.circular(100)),
                                            child: Text(subject.code, style: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.primaryText, fontSize: 12)),
                                          ),
                                          // Show anomaly badge only if AI flagged it — murag warning sign
                                          if (isAnomaly)
                                            Container(
                                              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                                              decoration: BoxDecoration(color: AppColors.warning.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(100)),
                                              child: const Row(
                                                mainAxisSize: MainAxisSize.min,
                                                children: [
                                                  Icon(Icons.auto_graph, color: AppColors.warning, size: 14),
                                                  SizedBox(width: 4),
                                                  Text('System Anomaly', style: TextStyle(color: AppColors.warning, fontWeight: FontWeight.w700, fontSize: 11)),
                                                ],
                                              ),
                                            ),
                                        ],
                                      ...
                                      Text(subject.name, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 18, letterSpacing: -0.3, color: AppColors.textPrimary), overflow: TextOverflow.ellipsis),
                                      ...
                                      const Text('Score vs. Department Average', style: TextStyle(color: AppColors.textSecondary, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
                                      const SizedBox(height: 10),
                                      Stack(
                                        children: [
                                          // Full-width soft vanilla background track
                                          Container(
                                            height: 12,
                                          ...
                                          FractionallySizedBox(
                                            ...
                                            child: Container(
                                              height: 12,
                                              decoration: BoxDecoration(
                                                // Warning gradient if anomaly, brand orange gradient if normal
                                                gradient: LinearGradient(
                                                  colors: isAnomaly
                                          ...
                                          // (80 = screen padding 20×2 + card padding 20×2)
                                          Positioned(
                                            left: (MediaQuery.of(context).size.width - 80) * (subject.deptAvg / 5.0).clamp(0.0, 1.0),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          Text('Dept Avg: ${subject.deptAvg}', style: const TextStyle(color: AppColors.textPrimary, fontSize: 11, fontWeight: FontWeight.w700)),
                                      ...
                                      if (isAnomaly) ...[
                                        ...
                                        Container(
                                          ...
                                          child: Text(
                                            ...
                                            style: const TextStyle(fontSize: 12, color: AppColors.warning, fontWeight: FontWeight.w600, height: 1.4),
                                          ),
                                        ),
                                      ],
                                      ...
                                      const Center(
                                        child: Text('Tap to view instructor breakdown', style: TextStyle(color: AppColors.primaryText, fontSize: 11, fontWeight: FontWeight.w700)),
                                      ),
                                    ],
                                  ),
                                ),
    ...
    return Expanded(
      child: Column(
        ...
        children: [
          ...
          Text(value, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15, color: color), overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
