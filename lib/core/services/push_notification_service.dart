...
class PushNotificationService {
  ...
  static Future<void> _showForeground(RemoteMessage message) async {
    ...
    // Suppress validation-related notifications if the gatherer toggle is off
    final title = notification.title ?? '';
    if (title == 'Scan Needs Review' || title == 'Import Error') {
      final prefs = await SharedPreferences.getInstance();
      final wantsValidation = prefs.getBool('gatherer_validation_notifications') ?? true;
      if (!wantsValidation) {
        debugPrint('Validation notification suppressed (toggle off): $title');
        return;
      }
    }
