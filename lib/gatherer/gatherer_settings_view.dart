...
class _GathererSettingsViewState extends State<GathererSettingsView> {
  ...
  bool _validationNotifications = true;
  static const _validationNotifPrefKey = 'gatherer_validation_notifications';
  ...
  Future<void> _loadValidationNotifPref() async {
    final prefs = await SharedPreferences.getInstance();
    if (mounted) setState(() => _validationNotifications = prefs.getBool(_validationNotifPrefKey) ?? true);
  }

  Future<void> _setValidationNotif(bool val) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_validationNotifPrefKey, val);
    if (mounted) setState(() => _validationNotifications = val);
  }
          ...
          children: [
            ...
            Card(
              ...
              child: _buildSwitchTile(
                ...
                subtitle: 'Get notified when a scan is flagged or has import errors.',
                icon: Icons.notifications_active_outlined,
                value: _validationNotifications,
                onChanged: _setValidationNotif,
              ),
            ),
            const SizedBox(height: 32),
